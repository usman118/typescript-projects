...
console.log(chalk.red(figlet.textSync("Node ATM", {
    verticalLayout: "full",
})));
let atm = new Atm();
...
inquirer.prompt(input_params).then(async (answers) => {
    let user = users.find((user) => user.name === String(answers.name) && user.pin === parseInt(answers.pin));
    if (user) {
            ...
            case "Deposit":
                {
                    ...
                    console.log(chalk.greenBright(`Your New  balance is: ${totalAmount}`));
                }
                break;
            case "Withdraw":
                {
